...
const Postures = () => {
  ...
  const [selectedMuscleGroup, setSelectedMuscleGroup] = useState('All');
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [selectedPosture, setSelectedPosture] = useState(null); // State for selected posture details
        ...
        {filteredPostures.length > 0 ? (
          filteredPostures.map((posture, index) => (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <LazyImage src={posture.path} alt={posture.name} onClick={() => setSelectedPosture(posture)} />
              <div className="posture-overlay">
                <h2>{posture.name}</h2>
                <p>{posture.muscleGroup}</p>
              </div>
      ...
      {selectedPosture && (
        ...
        >
          ...
          >
            ...
            <LazyImage src={selectedPosture.path} alt={selectedPosture.name} />
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              className="tip-text"
            >
              <strong>Tips:</strong> {selectedPosture.tips}
            </motion.p>
            <motion.button
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
